...
def cov_stats_to_html_table(
        sample_depth_info: Dict[str, mosdepth.MosdepthDepthInfo], low_coverage_threshold: int = 5
...
def flu_cov_stats_to_html_table(
        sample_depth_info: Dict[str, Dict[str, mosdepth.FluMosdepthDepthInfo]],
    ...
    for sample, mosdepth_info in sample_depth_info.items():
        df = pd.DataFrame([x.dict() for x in mosdepth_info.values()])
