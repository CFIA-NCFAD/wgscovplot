    ...
    return [{
        ...
        formatter: function (params) {
            ...
            if (i < samples.length){
                ...
                output += "<b>" + sample + "</b><br/>" 
                output += "<b>"+ "Position Info:"+ "</b><br/>"
                output += '- Position: ' + position.toLocaleString() + '<br/>'
                if (depth == 1E-6){
                    output += '- Coverage Depth: ' + parseInt(depth).toLocaleString() + '<br/>';
                } 
                else{
                    output += '- Coverage Depth: ' + depth.toLocaleString() + '<br/>';
                } 
                if (variants[i].hasOwnProperty(position)){
                    output += '- Ref: ' + ref_seq.substring(position - 1, position - 1 + variants[i][position].length) + '<br/>'
                    output = output + '- Variant: ' + variants[i][position] + '<br/>'
                }
                else{
                    output += '- Ref: ' + ref_seq[position-1] + '<br/>'
                }
                output += "<b>"+ "Coverage View Stats:"+ "</b><br/>"
                output += '- Range: ' + start_pos.toLocaleString() + ' - ' + end_pos.toLocaleString() + '<br/>';
