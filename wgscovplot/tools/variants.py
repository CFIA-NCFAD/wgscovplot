...
variants_cols = [
    ...
    ("REF_DP", "Reference Allele Depth", "Read depth of coverage supporting reference allele at reference position"),
    (
        "ALT_DP",
    ...
    (
        "DP",
        "Total Depth",
        "Total depth of coverage at reference position",
    ),
    (
        "ALT_FREQ",
...
variant_summary_cols = [
    ...
    ("min_depth", "Min Depth", "Minimum depth in all samples that the mutation is observed at."),
    ("max_depth", "Max Depth", "Maximum depth in all samples that the mutation is observed at."),
    ("mean_depth", "Mean Depth", "Mean/average depth that the mutation is observed at."),
    ("min_af", "Min AF", "Minimum alternate allele frequency of mutation in all samples."),
    ("max_af", "Max AF", "Maximum alternate allele frequency of mutation in all samples."),
    ("mean_af", "Mean AF", "Mean/average alternate allele frequency of mutation in all samples."),
    ("nt_pos", "Nucleotide Position", "Nucleotide position of mutation with respect to reference genome."),
    ("aa_pos", "Amino Acid Position", "Amino acid position of mutation in reference genome gene."),
]
...
BCFTOOLS_STATS_SAMPLE_NAME_CLEANUP = [re.compile(r"\.AF0\.\d+\.bcftools_stats.txt$")]
...
def simplify_snpsift(df: pd.DataFrame, sample_name: str) -> Optional[pd.DataFrame]:
    ...
    for c in df.columns:
        if c == "AC":
            ...
            REF_AC = df_ac[0].astype(int)  # noqa: N806
            REF_AC.name = "REF_AC"
            ALT_AC = df_ac[1].astype(int)  # noqa: N806
            ALT_AC.name = "ALT_AC"
            AF = ALT_AC / (REF_AC + ALT_AC)  # noqa: N806
            AF.name = "AF"
            series += [REF_AC, ALT_AC, AF]
            continue
        if c == "SR":
            df_ac = df[c].str.split(",", expand=True)
            REF_AC = df_ac[0].astype(int) + df_ac[1].astype(int)  # noqa: N806
            REF_AC.name = "REF_AC"
            ALT_AC = df_ac[2].astype(int) + df_ac[3].astype(int)  # noqa: N806
            ALT_AC.name = "ALT_AC"
            AF = ALT_AC / (REF_AC + ALT_AC)  # noqa: N806
            AF.name = "AF"
            series += [REF_AC, ALT_AC, AF]
...
def parse_ivar_vcf(
    ...
    if not sample_name:
        ...
        if sample_name is None:
            msg = f"Sample name is not defined for VCF: shape={df.shape}; columns={df.columns}"
...
def merge_vcf_snpsift(
    ...
    if df_snpsift is None and df_vcf is not None:
...
def get_info(
    ...
    for sample, vcf_path in sample_vcf.items():
        ...
        if variant_caller.startswith(VariantCaller.iVar.value):
            ...
            if df_nanopolish_vcf is not None:
                ...
                logger.warning(f'Sample "{sample}" has no entries in VCF "{vcf_path}"')
        elif variant_caller.startswith(VariantCaller.clair3.value):
            df_clair3_vcf = parse_clair3_vcf(df_vcf, sample)
    ...
    sample_snpsift = find_file_for_each_sample(
        basedir=basedir,
    ...
    for sample in all_samples:
        df_snpsift = sample_dfsnpsift.get(sample)
        df_vcf = sample_dfvcf.get(sample)
